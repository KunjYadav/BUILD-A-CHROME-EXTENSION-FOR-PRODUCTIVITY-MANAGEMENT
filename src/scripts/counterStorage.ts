...
import {
  CounterOverwriteEvent,
  CounterTimespanInterval,
  MsgEvent,
} from './types';
...
export default class CounterStorage {
  ...
  static async get(
    interval: CounterTimespanInterval = [new Date(), new Date()]
  ): Promise<Counter> {
    ...
    while (!isAfter(currentDate, interval[1])) {
      ...
      if (data) {
        ...
        for (let [url, time] of Object.entries(data.websiteTime)) {
          accumalativeCounter.websiteTime[url] = accumalativeCounter
            .websiteTime[url]
            ? accumalativeCounter.websiteTime[url] + time
            : time;
        }
      }
  ...
  static onOverwrite(
    callback: (counterData: CounterDailyData | null) => void
  ): void {
  ...
  private static async sendOverwriteEvent(
  ...
  static async mergeStorage(newData: unknown): Promise<void> {
    ...
    Object.keys(updatedCounterData).forEach((key) => {
      ...
      updatedCounterData[key].netTime =
        newCounterData[key].netTime + oldCounterData[key].netTime;
      ...
      Object.keys(updatedWebsiteTime).forEach((websiteKey) => {
        ...
        updatedWebsiteTime[websiteKey] =
          newWebsiteTime[websiteKey] + oldWebsiteTime[websiteKey];
      });
    });
