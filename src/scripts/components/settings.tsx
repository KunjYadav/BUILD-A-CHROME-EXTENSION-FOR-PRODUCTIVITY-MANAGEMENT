...
const Settings = () => {
  ...
  const exportData = async () => {
    ...
    setNotification({
      ...
      message: 'Successfully exported time data',
    });
  };
  ...
  const overwriteData = async () => {
    if (
      !window.confirm(
        'Are you sure you want to overwrite the current time data? This process is not reversible.'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setNotification({
        type: NotificationTypeEnum.SUCCESS,
        message: 'Successfully overwriten time data',
      });
    } catch (error) {
      setNotification({
        type: NotificationTypeEnum.ERROR,
        message: `Failed to overwrite data: ${error.message}`,
      });
    }
  };
  ...
  const mergeData = async () => {
    if (
      !window.confirm(
        'Are you sure you want to add time data to the current data? This process is not reversible.'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setNotification({
        type: NotificationTypeEnum.SUCCESS,
        message: 'Successfully merged time data',
      });
    } catch (error) {
      setNotification({
        type: NotificationTypeEnum.ERROR,
    ...
    <div className='settings'>
      ...
      {settings && (
        <form>
          ...
          <select
            name='idleTimer'
            value={settings.idleTimer as string}
          ...
          <input
            type='checkbox'
            name='videoCheck'
            checked={settings.videoCheck as boolean}
            onChange={handleChange}
          />
          <label htmlFor='videoCheck'>
            Continue counting time when watching a video
          </label>
          <br />
          <h2>Notifications</h2>
          <input
          ...
          <label htmlFor='notifications'>
            Notify me when I spend too much time on one site
          </label>
          <br />
          <div className={`${!settings.notifications ? 'disabled' : ''}`}>
            ...
            <select
              ...
              value={settings.notificationTimer as string}
              onChange={handleChange}
            >
          ...
          <button className='button' type='button' onClick={overwriteData}>
            Import (overwrite current time)
          </button>
          <button className='button' type='button' onClick={mergeData}>
            Import (add to current time)
          </button>
        </form>
      )}
      <footer>
        made by{' '}
        <a href='https://github.com/KunjYadav' style={{ color: '#227C9D' }}>
          Kunj
        </a>{' '}
        |{' '}
        <a
          href='https://github.com/KunjYadav/BUILD-A-CHROME-EXTENSION-FOR-PRODUCTIVITY-MANAGEMENT'
          style={{ color: '#17C3B2' }}
        >
          source code
        </a>{' '}
