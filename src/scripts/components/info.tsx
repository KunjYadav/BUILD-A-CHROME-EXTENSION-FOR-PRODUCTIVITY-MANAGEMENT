...
export default class Info extends React.Component<InfoProps, InfoState> {
  ...
  render() {
        ...
        <div className='timeSelect timespan'>
          <NamedField title='From' centerTitle>
            <DatePicker
              selected={this.state.dateInterval[0]}
              onChange={(date: Date) =>
                this.onDateChange([date, this.state.dateInterval[1]])
              }
              includeDates={this.state.availableDates}
            />
          </NamedField>
          ...
          <NamedField title='To' centerTitle>
            <DatePicker
              ...
              onChange={(date: Date) =>
                this.onDateChange([this.state.dateInterval[0], date])
              }
              includeDates={this.state.availableDates}
    ...
    const appSidebar = this.state.counter
      ? this.state.counter
          .sort()
          .map((data: WebsiteData, index: number) => (
    ...
    return (
      <div className='info'>
        <div className='left'>
          <div className='tabButtons'>{tabButtons}</div>
          {timespanSelect}
          <TimeCircle
            netTime={this.state.counter ? this.state.counter.netTime : null}
            subtitle={
              isEqual(this.state.dateInterval[0], this.state.dateInterval[1])
                ? Utils.formatDateShort(this.state.dateInterval[0])
                : `${Utils.formatDateShort(
                    this.state.dateInterval[0]
                  )} - ${Utils.formatDateShort(this.state.dateInterval[1])}`
            }
            mostUsedSites={
              this.state.counter ? this.state.counter.mostUsed() : null
            }
          />
        </div>
        <div className='right'>
          <NamedField title='App time'>
            <div className='appTime'>{appSidebar}</div>
          </NamedField>
        </div>
        <button
          className='smallButton refresh'
          onClick={() => {
            this.refresh();
          }}
        >
