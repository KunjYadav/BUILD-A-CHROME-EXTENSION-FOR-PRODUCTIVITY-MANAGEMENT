  ...
  static isValidCounterData(data: unknown): boolean {
    ...
    Object.entries(data).forEach(([key, value]: [string, any]) => {
      if (!/^\d{4} \d{1,2} \d{1,2}$/.test(key)) {
        throw new Error(
          `Invalid top-level key format: "${key}". Expected "YYYY M D".`
        );
      }
      ...
      if (
        ...
        throw new Error(
          `The "websiteTime" key under "${key}" must be an object with numeric values.`
        );
      } else {
        const totalWebsiteTime = Object.values(websiteTime).reduce(
          (sum, time) => (sum as number) + (time as number),
          0
        );
      ...
      if (
        ...
        throw new Error(
          `The "colors" key under "${key}" must match the expected colors.`
        );
      }

      if (otherColor !== expectedOtherColor) {
        throw new Error(
          `The "otherColor" key under "${key}" must match the expected value.`
        );
      }
    });
