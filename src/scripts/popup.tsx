...
class Popup extends React.Component<PopupProps, PopupState> {
  ...
  async componentDidMount() {
    let counter = await CounterStorage.get(
      this.timespans[this.state.selectedTab].interval
    );
  ...
  render() {
    const tabButtons = this.timespans.map(
      (timespan: CounterTimespanData, index: number) => (
        <TabButton
          key={index}
          tabIndex={index}
    ...
    const mostUsedSites = this.state.counter
      ? this.state.mostUsedSites.map((value: WebsiteData, index: number) => (
          <AppTime key={index} websiteData={value} />
        ))
      : null;
    ...
    return (
      <div className='popup'>
        ...
        />
        <div className='appTime'>{mostUsedSites}</div>
        <button
