...
async function iterateCounter(counter: Counter): Promise<Counter> {
  ...
  counter.websiteTime[hostname] = counter.websiteTime[hostname]
    ? counter.websiteTime[hostname] + 1
    : 1;
  ...
  if (
    ...
    website % parseInt(settings.notificationTimer as string) === 0
  ) {
    browser.notifications.create({
      type: 'basic',
      iconUrl: browser.runtime.getURL('assets/icons/256px.png'),
      title: 'Time Tracker & Productivity Management',
      message: `You have already spent ${Utils.formatTime(
        website
      )} on ${hostname}!`,
    });
  }
...
async function main(): Promise<void> {
  ...
  CounterStorage.onOverwrite((counterData: CounterDailyData | null) => {
    counter = counterData
      ? Counter.constructFromDailyData(counterData)
      : new Counter();
